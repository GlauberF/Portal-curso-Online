...
class Compra{
	...
	protected function setnomeVenda($nomeVenda){
	...
	protected function setliberacao($liberacao){
		$this->liberacao = $liberacao;
	}	
	
	public function all($id, $aluno){
		...
		if(!is_null($id)){			
			...
			$sql = "SELECT `compra`.`id`, `compra`.`data`, `compra`.`modo_pag`, `compra`.`valor`, `compra`.`status`, `compra`.`cliente_idcliente`, `compra`.`notification_id`
					FROM `compra` WHERE `aluno_idaluno` = $aluno ORDER BY `compra`.`id` ASC;";			
		}
		$vai = new MySQLDB();

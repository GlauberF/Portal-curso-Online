<?php
	...
	if(isset($_REQUEST['action'])){
		switch ($_REQUEST['action']) {
			case "out":
				$cad = new Control();
				$cad->delAula($_REQUEST["nu"], $_REQUEST["ed"]);
			break;
		}
	}
														...
														<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
															...
															<tbody>
																<?php
																	...
																	$c = new Control();
																	$arr = $c->Controle_aula($id = null, $_GET['ed']);
																...
																<tr class="<? if($i % 2){ echo TABLE1; }else{ echo TABLE2; }?>">
																	...
																	<td>
																		<div class="actions">
																			<ul>
																				<li><a class="action2" href="vi_aula.php?nu=<?=$arr[$i]->idaula['idaula']?>&ed=<?=$_GET['ed']?>">2</a></li>
																				<li><a class="action4" href="aula.php?action=out&nu=<?=$arr[$i]->idaula['idaula']?>&ed=<?=$_GET['ed']?>" onClick="return confirm('Deletar a aula <?php print $arr[$i]->nome['nome']; ?>?')">4</a></li>
																			</ul>
																		</div>
																	</td>
																</tr>
										...
										<div class="sct_left">
											<div class="sct_right">
												<ul class="sidebar_menu">
													<li><a href="cad_aula.php?ed=<?=$_GET['ed']?>">Cadastrar nova aula</a></li>								
												</ul>
											</div>
										</div>
									</div>

<?php
	...
	if(isset($_REQUEST['action'])){
		switch ($_REQUEST['action']) {
			case "in":
				$cad = new Control();
				$cad->cadMaterial($_POST["nome"], $_FILES["endereco"], $_POST["descricao"], $_POST["aula_idaula"]);
			break;
			case "edit":
				$cad = new Control();
				$cad->editMaterial($_POST["idmaterial"], $_POST["nome"], $_POST["descricao"], $_POST["aula_idaula"]);
			break;
			case "out":
				$cad = new Control();
				$cad->delMaterial($_REQUEST["nu"]);
			break;
			case "arq":
				$cad = new Control();
				$cad->Arquivo($_POST["idmaterial"], $_FILES["endereco"]);
			break;			
		}
	}
				...
				<div id="sec_menu">
					<ul>
						<?php if(isset($_GET['ed'])){ ?>
							<li><a href="material.php" class="sm5">Voltar</a></li>
						<?php }else{ ?>													
									...
									<div class="sct_right">
										<div class="sct_left">
											...
											?>
												...
												<form id="curso" action="material.php?action=edit" method="POST" class="search_form general_form" enctype="multipart/form-data">
													<fieldset>
														<div class="forms">
															<div class="row">
																<label>Nome:</label>
																<div class="inputs">
																	<span class="input_wrapper"><input class="text" name="nome" id="nome" value="<?=$arr[$i]->nome['nome']?>" type="text" title="O nome não pode ficar em branco." /><input name="idmaterial" id="idmaterial" value="<?=$arr[$i]->idmaterial['idmaterial']?>" type="hidden" /></span>
														...
														<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
															...
															<tbody>
																<?php
																	...
																	$c = new Control();
																	$arr = $c->Controle_material($id = null);
																	for ($i = 0; $i < count($arr); $i++){
																...
																<tr class="<? if($i % 2){ echo TABLE1; }else{ echo TABLE2; }?>">
																	...
																	<td>
																		<div class="actions">
																			<ul>
																				<li><a class="action2" href="material.php?ed=<?=$arr[$i]->idmaterial['idmaterial']?>">2</a></li>
																				<li><a class="action4" href="material.php?action=out&nu=<?=$arr[$i]->idmaterial['idmaterial']?>" onClick="return confirm('Deletar o material <?php print $arr[$i]->nome['nome']; ?>?')">4</a></li>
																			</ul>
																		</div>
																	</td>
																</tr>
				...
				?>				
					<div class="quick_info">
						...
						<div class="quick_info_content">
							...
							<dl>
								<dt>Filesize:</dt>
								<dd><?=filesize("../../material/" . $_SESSION['FolderCliente'] . "/" . $arr[$i]->endereco['endereco'])?> Bytes</dd>
							</dl>
							<dl>
					...
					<div style='display:none'>
						<div id='pdf' style='padding:10px; background:#fff;'>
							<h3>PDF</h3>
							<?php
								$file = "../../material/" . $_SESSION['FolderCliente'] . "/" . $arr[$i]->endereco['endereco'];
							?>
